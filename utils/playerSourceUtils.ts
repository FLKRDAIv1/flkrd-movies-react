...
export const getSourceUrl = (name: string, id: string, type: 'movie' | 'tv', season?: number, episode?: number, progress: number = 0, accentColor?: string, subtitleUrl?: string) => {
  ...
  switch (name) {
    ...
    case 'FLKRD SERVER 1': // VidLink Pro (TOP 2)
      const vlParams = `?primaryColor=${playerColor}&secondaryColor=5c4747&iconColor=eefdec&icons=vid&player=jw&title=true&poster=true&autoplay=true&nextbutton=true&server=2${progress > 10 ? `&startAt=${Math.floor(progress)}` : ''}${subtitleUrl ? `&sub_file=${encodeURIComponent(subtitleUrl)}&sub_label=Kurdish` : ''}`;
      return isTv
