...
export const getSourceUrl = (name: string, id: string, type: 'movie' | 'tv', season?: number, episode?: number, progress: number = 0, accentColor?: string, subtitleUrl?: string) => {
  ...
  switch (name) {
    ...
    case 'FLKRD SERVER 1': // VidLink Pro (TOP 2)
      const vlParams = `?primaryColor=${playerColor}&secondaryColor=a2a2a2&iconColor=eefdec&icons=default&player=default&title=true&poster=true&autoplay=false&nextbutton=true${progress > 10 ? `&startAt=${Math.floor(progress)}` : ''}${subtitleUrl ? `&subtitle=${encodeURIComponent(subtitleUrl)}&subtitleLabel=Kurdish` : ''}`;
      return isTv
