  ...
  return (
    <>
      ...
      <Portal id="expandable-profile-card-portal">
        <AnimatePresence>
          {isOpen && (
            <div className="fixed inset-0 z-[120000] flex items-center justify-center p-4 overflow-hidden pointer-events-none">
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              <motion.div
                ...
                className="relative w-full max-w-4xl h-[80vh] bg-card rounded-2xl overflow-hidden border border-border z-10 flex flex-col md:flex-row shadow-xl pointer-events-auto"
              >
                <button 
                  onClick={() => setIsOpen(false)} 
                  className="absolute top-4 right-4 z-20 flex h-8 w-8 items-center justify-center bg-background/50 hover:bg-accent rounded-full border border-border text-foreground transition-colors backdrop-blur-sm cursor-pointer"
                >
                  ...
                  >
                    {content || (
                      <div className="flex flex-col gap-6">
                        ...
                        <button className="mt-4 px-5 py-2.5 bg-primary text-primary-foreground font-medium rounded-lg hover:opacity-90 transition-opacity self-start shadow-sm cursor-pointer">
                          Connect with Jane
                        </button>
                      </div>
                    )}
