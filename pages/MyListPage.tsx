                ...
                <div className="flex items-center gap-4">
                    {myListItems.length > 0 && !isSharedView && (
                        <button 
                            ...
                            className="flex items-center gap-3 bg-white text-black font-black py-4 px-8 rounded-2xl hover:bg-gray-200 transition-all shadow-xl uppercase italic tracking-widest text-xs"
                        >
                            <Share size={18} />
                            Share Protocol
                        </button>
                    ...
                    {!isSharedView && myListItems.length > 0 && (
                        <button 
                            onClick={clearMyList}
                            className="p-4 bg-white/5 hover:bg-red-600/20 border border-white/10 rounded-2xl transition-all group"
                        >
            ...
            {myListItems.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-8">
                    {myListItems.map((item, index) => (
                        ...
                        >
                            <div className="aspect-[2/3] overflow-hidden rounded-[2.5rem] border-2 border-white/5 group-hover:border-red-600/50 group-hover:shadow-[0_0_30px_rgba(229,9,20,0.4)] shadow-2xl transition-all relative">
                                <img
                                ...
                                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                    <button
                                        ...
                                        className="p-2.5 rounded-xl bg-red-600 text-white shadow-xl"
                                    >
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">Watch Now</span>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
