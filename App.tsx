...
const DubbedMoviesPage = lazy(() => import('./pages/DubbedMoviesPage'));
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.success !== false) {
                const resolved = resolveKurdistanDistrict(data.city, data.region, data.country);
        ...
        if (res2.ok) {
            ...
            const resolved2 = resolveKurdistanDistrict(data2.city, data2.region, data2.country_name || data2.country);
            const info2: GeoLocationDetails = {
                country: data2.country_name || data2.country || 'Unknown',
                city: resolved2.city,
                district: resolved2.district
            };
    ...
    try {
        ...
        if (tz && (tz.includes('Baghdad') || tz.includes('Erbil') || tz.includes('Asia/Baghdad'))) {
            const fallback: GeoLocationDetails = {
                country: 'Iraq',
                city: 'هەولێر (Erbil)',
                district: 'قەزای مێرگەسۆر (Mergasor / Barzan)'
            };
...
const ProfileBackgroundVideo: React.FC<{ theme: string }> = ({ theme }) => {
    ...
    // Force play on mount and whenever src changes — required for iOS Safari autoplay policy
    useEffect(() => {
        ...
        const tryPlay = () => {
            vid.play().catch(() => {
                // Retry on first user gesture (iOS blocks autoplay until interaction)
                const retry = () => {
                    vid.play().catch(() => {});
                };
        ...
        // Slight delay to let the browser settle, especially on mobile
